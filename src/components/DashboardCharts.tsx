  ...
  return (
    <div className="dashboard-bento">
      <section className="kpi-grid" aria-label="Core ops KPIs">
        {kpis.length === 0
          ? ['New users', 'Tasks created', 'Workers registered'].map((label) => (
              <article key={label} className="metric metric-kpi" aria-busy="true">
                <p className="metric-label">{label}</p>
                <div className="skeleton skeleton-value" />
                <p className="meta">No Mongo count for this range</p>
              </article>
            ))
          : kpis.map((kpi) => <KpiCard key={kpi.key} kpi={kpi} animate={animate} />)}
      </section>

      {emptyReason && kpis.length === 0 && status.length === 0 && dau.length === 0 ? (
      ...
      {dau.length > 0 ? (
        <article className="section chart-card">
          ...
          <div className="chart-frame" role="img" aria-label="Daily active users trend">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={dau} margin={{ top: 12, right: 12, left: 0, bottom: 0 }}>
      ...
      {comparison.length > 0 ? (
        <article className="section chart-card">
          ...
          <p className="muted">
            Grouped bars for the three core KPIs. Numbers are also listed below.
          </p>
          <div
            className="chart-frame"
            role="img"
            aria-label="Current versus previous period for new users, tasks created, and workers registered"
          >
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={comparison}
              ...
              >
                ...
                <Bar
                  ...
                  isAnimationActive={animate}
                >
                  <LabelList dataKey="previous" position="top" fill={INK} fontSize={12} />
                </Bar>
                <Bar
                  dataKey="current"
                  name="Current"
                  ...
                  isAnimationActive={animate}
                >
                  <LabelList dataKey="current" position="top" fill={INK} fontSize={12} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
      ...
      {status.length > 0 ? (
        <article className="section chart-card">
          ...
          <p className="muted">Open marketplace snapshot — labels and counts, not color alone.</p>
          <div
